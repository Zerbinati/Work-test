...
Position* globalPos = nullptr; // Definition of the global pointer to the position
...
void setoption(std::istringstream& is) {
    ...
    if (Options.count(name))
        Options[name] = value;
    else if (name == "Shashin Style") {
        Eval::set_shashin_style(value);
    } else {
...
void UCI::loop(int argc, char* argv[]) {
    ...
    Position     pos;
	globalPos = &pos;  // Assign global pointer
