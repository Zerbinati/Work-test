...
bool Position::is_isolated(Square s, Bitboard pawns) const {
    File f = file_of(s);
    Bitboard adjacentFiles = (f > FILE_A ? file_bb(File(f - 1)) : 0) |
                             (f < FILE_H ? file_bb(File(f + 1)) : 0);
    return !(pawns & adjacentFiles);
}

bool Position::is_doubled(Square s, Bitboard pawns) const {
    return popcount(pawns & file_bb(file_of(s))) > 1;
}

Piece Position::mirrored_piece(Square s) const {
    Square mirrored = Square((7 - rank_of(s)) * 8 + file_of(s));
    return piece_on(mirrored);
}
...
bool Position::is_open_file(Square s) const {
    return !(pieces(PAWN) & file_bb(file_of(s)));
}

Bitboard Position::attacking_pieces(Color c, Square s) const {
    return attackers_to(s) & pieces(c);
}

int Position::king_safety_score(Color side) const {
    Square kingSquare = king_square(side);
    int score = 0;

    if (is_open_file(kingSquare))
        score -= 50;

    Bitboard attackers = attacking_pieces(~side, kingSquare);
    while (attackers) {
        pop_lsb(attackers);
        score -= 10;
    }
...
int Position::mobility_score() const {
    int score = 0;
    Bitboard b = pieces();
    while (b) {
        pop_lsb(b); // Rimuove un bit dal bitboard
        score += 1; // Incrementa lo score, o aggiungi logica specifica
    }
    return score;
}
...
int Position::pawn_structure_score() const {
    int score = 0;
    Bitboard pawns = pieces(side_to_move(), PAWN);
    while (pawns) {
        Square s = pop_lsb(pawns);
        if (is_isolated(s, pawns)) score -= 20;
        if (is_doubled(s, pawns)) score -= 10;
    }
    return score;
}

bool Position::is_symmetric() const {
    for (Square s = SQ_A1; s <= SQ_H8; ++s) {
        if (piece_on(s) != mirrored_piece(s)) {
            return false;
        }
    }
    return true;
...
bool Position::is_sacrifice(Move m) const {
    Piece moved = moved_piece(m);
    Piece captured = captured_piece();

    // Determine if a Move Constitutes a Sacrifice
    return PieceValue[moved] > (captured == NO_PIECE ? 0 : PieceValue[captured]);
}

// Check if a Move Leads to a Promising Attack
bool Position::leads_to_attack(Move m) const {
    Square to = m.to_sq(); // Destination square of the move.
    Bitboard attackers = attackers_to(to); // Get all attackers to the destination square.

    // Count the number of attackers from the side to move.
    int attackCount = popcount(attackers & pieces(side_to_move()));

    // Return true if there are at least 3 attackers.
    return attackCount > 2;
}

// Check if the Position Implies a Sacrifice
bool Position::is_sacrifice() const {
    Bitboard allPieces = pieces(side_to_move());       // Get all pieces of the side to move.
    Bitboard enemyPieces = pieces(~side_to_move());    // Get all enemy pieces.
    Bitboard attackingPieces;

    // Iterate over all pieces of the side to move.
    while (allPieces) {
        Square from = pop_lsb(allPieces); // Get the position of the current piece.
        Piece piece = piece_on(from);     // Identify the piece.

        // Generate pseudo-legal moves for the current piece.
        attackingPieces = attackers_to(from, pieces());

        // Check if the piece attacks enemy pieces and is of sufficient value.
        if (popcount(attackingPieces & enemyPieces) > 0 && PieceValue[piece] >= 300) {
            return true; // A promising sacrifice is detected.
        }
    }
    return false; // No sacrifices detected.
}
