...
LimitsType Limits;                          // Structure for search limits and constraints.
}

// Calculate Total Material in the Root Position
int Hypnos::Search::rootMaterial(const Position& rootPos) {
    int material = 0; // Initialize total material to zero.

    // Iterate through all squares on the board (explicit namespace access).
    for (Square sq : Hypnos::ALL_SQUARES) {
        Piece p = rootPos.piece_on(sq); // Get the piece on the current square.

        if (p != NO_PIECE) { // Skip empty squares.
            // Combine color and type to create a valid Piece.
            Color c = color_of(p);
            PieceType pt = type_of(p);
            Piece combinedPiece = make_piece(c, pt);

            // Add the material value of the piece to the total.
            material += Eval::piece_value(combinedPiece); // Use the correct Piece structure.
        }
    }
...
void Thread::search() {
    ...
    // Ensure alphaBetaScore is Defined Correctly
    Value alphaBetaScore = VALUE_ZERO; // Initialize alphaBetaScore to a default value.

    if (!rootMoves.empty()) { 
        alphaBetaScore = rootMoves[0].score; // Use the score of the best root move if available.
    }

    // Apply Shashin Dynamic Style if Enabled
    if (Options["Shashin Dynamic Style"]) {
        Eval::dynamic_shashin_style(rootPos, alphaBetaScore, Search::rootMaterial(rootPos));
    }

    // Determine Dynamic Phase of the Game
    int dynamicPhase = Eval::determine_dynamic_phase(rootPos);

    // Update NNUE Weights Dynamically if Enabled
    int talWeight = 30, petrosianWeight = 40, capablancaWeight = 30; // Default weights.

    if (Options["NNUE Dynamic Weights"]) {
        // Update weights based on dynamic phase.
        Eval::NNUE::update_weights(dynamicPhase, rootPos, talWeight, petrosianWeight, capablancaWeight);

        // Apply blended updates to weights.
        Eval::NNUE::update_weights_with_blend(rootPos, talWeight, petrosianWeight, capablancaWeight);
    }
}

    // Configure MultiPV and Skill Level
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        if (Limits.use_time_management() && !Threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            double totalTime = Time.optimum() * fallingEval * reduction * bestMoveInstability * EvalLevel[el] * contempt;
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (variety && std::abs(UCI::to_cp(bestValue)) < Options["Variety Max Score"])
    {
        ...
        if (bestValue + variety * PawnValue / 100 >= 0
